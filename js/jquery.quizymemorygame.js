...
(function ($) {
  ...
  var methods = {
    ...
    init: function (options) {
      ...
      var handleClick = function () {
        ...
        if (numClicks == 0) {
          ...
          if (tClass == selItemClass) {
            opts.matchingElementsFn({$elem: $('.' + tClass)});
            showResIcon('correct', tClass, tId);
            unbindClick($('.' + tClass));
            ...
            if (numMatches == itemsNum / 2) {
              ...
              if (opts.gameSummary) {
                ...
                opts.gameSummaryFn({ clicks: numTotalClicks, time: numSeconds });
            ...
            opts.unMatchingElementsFn({$elem: $('.' + tClass)});
            showResIcon('wrong', '', '');
      ...
      var showResIcon = function (type, tClass, tId) {
        if (opts.resultIcons) {
          var el;
          var time = Math.round(delayShow / 3);
          if (type == 'wrong') {
            el = $('div#quizy-mg-msgwrong');
            el.delay(time).fadeIn(time / 2);
            el.delay((time/2) * 3).fadeOut(time/3);
          } else if (type == 'correct') {
            // get id
            var msg = msgHtml[tClass];
            var lastId = tId.slice(-1);
            if(lastId < opts.colCount){
            ...
            displayMsg(tId.substring(0, tId.length-1)+lastId, msg);

          }
        }
      }

      var displayMsg = function(idItem, msg){

        $('.bubble-info').remove();

        $('body').append('<div class="bubble-info"></div>');

        var topPos = $('#'+idItem).offset().top + opts.itemHeight;
        var leftPos =$('#'+idItem).offset().left - ((opts.itemWidth/2)+opts.bubbleInfoMarginleft);
        var time = Math.round(delayShow / 3);
        ...
        $('.bubble-info').css({
          ...
          top: topPos+'px',
          left: leftPos+'px',
          background: 'url("'+msg+'") no-repeat',
          height: opts.bubbleInfoHeight+'px',
          width: opts.bubbleInfoWidth+'px'
        });

        $('.bubble-info').fadeIn(time);
        $('.bubble-info').click(function(){
          $(this).remove();
        });
      ...
      while (i < itemsNum) {
        ...
        if (i >= opts.colCount) {
          ...
          msgHtml[inEl.attr('class')] = inEl.attr('data-msg');
        }

        // Adds the innerHtml to the array
        inHtml[j] = inEl.children('span').attr('data-img');

        // appends the cards to the element
        $(this).append('<div id="' + itemsClass + j + '" class="' + itemsClass + complClass +
          '" style="width:' +
      ...
      if (opts.gameSummary) {
        ...
        $('div#gs-closebut').click(function () {
          $(this).parent().fadeOut();
          $('.bubble-info').remove();
          location.href = opts.endLocationHref;
        });
  ...
  $.fn.quizyMemoryGame.defaults = {
    ...
    resultIcons: true,
    gameSummary: true,
    textSummaryTitle: 'Bravo !',
    replayButton: false,
    replayButtonText: '',
    closeButtonText: 'Terminer',
    textSummaryClicks: 'clicks',
    textSummaryTime: 'secondes',
    onFinishCall: '',
    endLocationHref: '#!/dashboard',
    bubbleInfoWidth: 340,
    bubbleInfoHeight: 190,
    bubbleInfoMarginleft: 20,
    gameSummaryFn: function () {
    },
    matchingElementsFn: function () {
    },
    unMatchingElementsFn: function () {
    }
  }
